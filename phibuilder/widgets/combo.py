...
class M3ComboBox(QComboBox):
    def __init__(self, items: list[str] | None = None, theme: Theme | None = None, parent=None):
        ...
        # 32 px — aligné sur ds.field_height (le QSS height s'applique au contenu,
        # la bordure s'ajoute : height 30 + 2 = 32)
        self.setMinimumHeight(_SCALE.spacing(SpacingToken.LG))
        if items:
            self.addItems(items)
        self._update_style()
...
M3ComboBox {{ padding: 0 {p}px; height: {s.spacing(SpacingToken.LG) - s.spacing(SpacingToken.XXS) // 2}px; border: 1px solid {c.outline}; border-radius: {r}px;
...
M3ComboBox::drop-down {{ border: none; width: {s.spacing(SpacingToken.LG)}px; }}
